...
def test_crossref_item_to_candidate_normalizes_doi_and_metadata():
    ...
    candidate = smeli.candidates._crossref_item_to_candidate(item)
...
def test_openalex_item_to_candidate_keeps_doi_less_arxiv_records():
    ...
    candidate = smeli.candidates._openalex_item_to_candidate(item)
...
def test_datacite_record_to_candidate_extracts_attributes():
    ...
    candidate = smeli.candidates._datacite_record_to_candidate(record)
...
def test_arxiv_entry_to_candidate_parses_atom_entry():
    ...
    candidate = smeli.candidates._arxiv_entry_to_candidate(entry)
    assert candidate["arxiv_id"] == "2507.11521"
...
def test_candidate_matching_strict_loose_and_all_fields():
    ...
    assert smeli.candidates._candidate_matches(candidate, title="opinion dynamics", author="starnini", year="2025")
    assert not smeli.candidates._candidate_matches(candidate, title="network science", author="starnini")
    assert smeli.candidates._candidate_matches(candidate, title="dynamics beyond", allow_loose_title=True)
    assert smeli.candidates._candidate_matches_loose_query(candidate, "starnini opinion dynamics")
